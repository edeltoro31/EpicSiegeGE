...
public class SpriteBat {
	   ...
	   GL20 gl;                                           // GL Instance
	   Vertixes vertices;                                 // Vertices Instance Used for Rendering
	   ...
	   public SpriteBat(GL20 gl, int maxSprites)
	   {
	      this.gl = gl;                                   // Save GL Instance
	      this.vertexBuffer = new float[maxSprites * VERTICES_PER_SPRITE * VERTEX_SIZE];  // Create Vertex Buffer
	      this.vertices = new Vertixes( gl, maxSprites * VERTICES_PER_SPRITE, maxSprites * INDICES_PER_SPRITE, false, true, false );  // Create Rendering Vertices
	   ...
	   {
	      gl.glBindTexture( GL20.GL_TEXTURE_2D, textureId );  // Bind the Texture
	   ...
	   public void endBatch()
	   {
	      if ( numSprites > 0 )   // IF Any Sprites to Render
	      {
	         ...
	         //vertices.draw( GL20.GL_TRIANGLES, 0, numSprites * INDICES_PER_SPRITE );  // Render Batched Sprites
	   ...
	   public void drawSprite(float x, float y, float width, float height, TextureRegion region, float z)
	   {
	      ...
	      vertexBuffer[bufferIndex++] = region.getU();        // Add U for Vertex 0
	      vertexBuffer[bufferIndex++] = region.getV2();        // Add V for Vertex 0
	      ...
	      vertexBuffer[bufferIndex++] = region.getU2();        // Add U for Vertex 1
	      vertexBuffer[bufferIndex++] = region.getV2();        // Add V for Vertex 1
	      ...
	      vertexBuffer[bufferIndex++] = region.getU2();        // Add U for Vertex 2
	      vertexBuffer[bufferIndex++] = region.getV();        // Add V for Vertex 2
	      ...
	      vertexBuffer[bufferIndex++] = region.getU();        // Add U for Vertex 3
	      vertexBuffer[bufferIndex++] = region.getV();        // Add V for Vertex 3
